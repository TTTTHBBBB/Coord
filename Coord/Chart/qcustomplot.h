...
/*!
  QCP 命名空间包含用于整个 QCustomPlot
  库中使用的通用枚举、QFlags 和函数。

  它通过 QCP::staticMetaObject 为其枚举和标志提供基于 QMetaObject 的反射。
*/
...
/*!
  定义导出功能中指定图像分辨率的不同单位。功能中指定图像分辨率的不同单位。

  \see QCustomPlot::savePng, QCustomPlot::saveJpg, QCustomPlot::saveBmp, QCustomPlot::saveRastered
*/
enum ResolutionUnit { ruDotsPerMeter       ///< Resolution is given in dots per meter (dpm) 分辨率以（dpm）表示
...
/*!
  ...
  定义导出时如何处理外观笔（数字宽度为 0 的笔）。
  \see QCustomPlot::savePdf
*/
enum ExportPen { epNoCosmetic     ///< 导出时将化妆笔转换为像素宽度为 1 的笔
                 ,epAllowCosmetic ///< 正常导出化妆笔（例如，在 PDF 导出中，化妆笔在屏幕上始终显示为 1 个像素，与查看器的缩放级别无关）
               };

/*!
  代表负号和正号域，例如用于传递给
  QCPAbstractPlottable::getKeyRange 和 \ref QCPAbstractPlottable::getValueRange。
  这主要是在处理对数轴刻度时需要的，因为在同一时间只能看到一个符号域同时可见。
*/
enum SignDomain { sdNegative  ///< 负号域，即小于零的数字
                  ,sdBoth     ///< 两个符号域，包括零，即所有数字
                  ,sdPositive ///< 正符号域，即大于零的数字
                };
...
/*!
  ...
  定义可以强制绘制抗锯齿/不抗锯齿的绘图对象。如果一个对象,对象既不是强制绘制为反锯齿，
  也不是强制绘制为非反锯齿，那么如何绘制将由相应的元素决定如何绘制。通常情况下，它会为此提供一个 setAntialiased 函数。
...
/*!
  定义绘图提示，控制绘图质量和速度的各个方面。
  \see QCustomPlot::setPlottingHints
*/
enum PlottingHint { phNone              = 0x000 ///< <tt>0x000</tt> No hints are set 未设置提示
                                                //图形/曲线的绘制采用更快的方法。这种方法会降低线段的质量，尤其是线段连接的质量。仅用于实线笔。
                    ,phFastPolylines    = 0x001 ///< <tt>0x001</tt> Graph/Curve lines are drawn with a faster method. This reduces the quality especially of the line segment
                                                ...
                                                /*
                                                 * 当调用参数为 ref\ QCustomPlot::rpRefreshHint 的 QCustomPlot::replot() 时，会立即重绘()而不是软更新()。默认设置该参数是为了防止连续快速重绘时（例如用户用鼠标拖动范围）冻结绘图。
                                                */
                    ,phImmediateRefresh = 0x002 ///< <tt>0x002</tt> causes an immediate repaint() instead of a soft update() when QCustomPlot::replot() is called with parameter \ref QCustomPlot::rpRefreshHint.
                                                ...
                                                //轴（刻度线）标签将作为像素图缓存，从而提高重绘性能。
                    ,phCacheLabels      = 0x004 ///< <tt>0x004</tt> axis (tick) labels will be cached as pixmaps, increasing replot performance.
                  };
Q_DECLARE_FLAGS(PlottingHints, PlottingHint)

/*!
  定义 QCustomPlot 可实现的鼠标交互。
  \c Interactions is a flag of or-combined elements of this enum type.
  
  \see QCustomPlot::setInteractions
*/
enum Interaction { iNone              = 0x000 ///< <tt>0x000</tt> 不可能进行任何互动
                   ,iRangeDrag        = 0x001 ///< <tt>0x001</tt> 轴范围是可拖动的（参见 \ref QCPAxisRect::setRangeDrag, \ref QCPAxisRect::setRangeDragAxes)
                   ,iRangeZoom        = 0x002 ///< <tt>0x002</tt> 轴范围可以用鼠标滚轮缩放（参见 \ref QCPAxisRect::setRangeZoom， \ref QCPAxisRect::setRangeZoomAxes）。
                   ,iMultiSelect      = 0x004 ///< <tt>0x004</tt> 用户可以在点击的同时，按住通过 \ref QCustomPlot::setMultiSelectModifier 设置的修饰符来选择多个对象。
                   ,iSelectPlottables = 0x008 ///< <tt>0x008</tt> Plottables 是可选择的（如图形、曲线、条形图......见 QCPAbstractPlottable）
                   ,iSelectAxes       = 0x010 ///< <tt>0x010</tt> 轴可选择（或部分轴可选择，参见 QCPAxis::setSelectableParts）
                   ,iSelectLegend     = 0x020 ///< <tt>0x020</tt> 可选择图例（或其子项目，参见 QCPLegend::setSelectableParts）
                   ,iSelectItems      = 0x040 ///< <tt>0x040</tt> 项目是可选择的（矩形、箭头、文本项目等，请参见 \ref QCPAbstractItem)
                   ,iSelectOther      = 0x080 ///< <tt>0x080</tt> 所有其他对象均可选择（例如您自己的派生可分层对象、其他布局元素......）。
                   ,iSelectPlottablesBeyondAxisRect = 0x100 ///< <tt>0x100</tt> 在进行可绘图选择/命中测试时，该标记会将敏感区域扩展到轴矩形之外。
                 };
...
*/
enum SelectionRectMode { srmNone    ///< 选择矩形被禁用，所有鼠标事件都被转发到底层对象，例如轴范围拖动
                         ,srmZoom   ///< 拖动鼠标时，选择矩形将处于活动状态。释放时，当前设置为范围缩放轴（\ref QCPAxisRect::setRangeZoomAxes）的轴将相应缩放其范围。
                         ,srmSelect ///< 拖动鼠标时，选择矩形将被激活。释放鼠标时，如果可绘图板的可选择性设置允许，在选择矩形范围内的可绘图板数据点将被选中。(详见 \ref dataselection “数据选择机制”）。
                         ,srmCustom ///< 拖动鼠标时，选择矩形将处于活动状态。程序员有责任将相应的插槽连接到选择矩形的信号上（例如：\ref QCPSelectionRect::accepted），以便处理用户交互。
                       };

/*!
  定义了选择绘图板的不同方式。这些图片显示了不同选择类型的效果：
  <center>
  <table>
  <tr>
...
*/
enum SelectionType { stNone                ///< 不可选择绘图仪
                     ,stWhole              ///< Selection 的行为类似于 \ref stMultipleDataRanges，但如果有任何数据点被选中，则整个绘图板都会被绘制为被选中。
                     ,stSingleData         ///< 每次可选择一个单独的数据点
                     ,stDataRange          ///< 可选择多个连续数据点（数据范围）
                     ,stMultipleDataRanges ///< 可选择任何数据点/范围组合
                    };
...
/*! \internal
  ...
  返回指定的 \a 值是否被认为是 plottables 的无效数据值（即是 \e nan 还是 \e +/-inf）。
  是 \e nan 还是 \e +/-inf）。当
  编译器标志 \c QCUSTOMPLOT_CHECK_DATA被设置。
*/
...
/*! \internal
  ...
  返回指定的 \a 边距的值（如果 \a 边距是\ref QCP::msNone 或 \ref QCP::msNone）。如果 \a side 是 \ref QCP::msNone 或
  \则返回 0。
...
public:
  ...
  QCPVector2D   perpendicular() const { return QCPVector2D(-mY, mX); }//返回一个垂直于传参的向量
...
public:
  /*!
    ...
    定义画家可以在其中运行的特殊模式。它们可禁用或启用某些子功能/修复/变通方法、
    取决于是否需要在相应的输出设备上使用。
  */
  enum PainterMode { pmDefault       = 0x00   ///< <tt>0x00</tt> 在屏幕设备上绘画的默认模式
                     ,pmVectorized   = 0x01   ///< <tt>0x01</tt> 矢量化输出，一般不需要额外的像素处理（如抗锯齿）
                     ,pmNoCaching    = 0x02   ///< <tt>0x02</tt> 确保每次绘图操作都是按照当前状态执行，不依赖于之前的绘图结果。
                     ,pmNonCosmetic  = 0x04   ///< <tt>0x04</tt>  QPainter 的笔触（线条宽度）不应该被视为“化妆品”，即不受到变换（例如缩放）的影响。
                   };
  Q_ENUMS(PainterMode)
...
protected:
  // property members:
  bool mVisible;
  QCustomPlot *mParentPlot;//存储当前的对象
  QPointer<QCPLayerable> mParentLayerable;//当前对象的父层级元素
  QCPLayer *mLayer;
  bool mAntialiased;
...
public:
  ...
  */
  ...
  double realLength() const;//返回各种类型的结束线段的长度
  ...
  */
  enum TickStepStrategy
  {
    tssReadability    ///< A nicely readable tick step is prioritized over matching the requested number of ticks (see \ref setTickCount)
                      ///优先考虑生成易于阅读的刻度步长，而不是严格匹配请求的刻度数量
    ,tssMeetTickCount ///< Less readable tick steps are allowed which in turn facilitates getting closer to the requested tick count
                      ///允许使用不那么易于阅读的刻度步长，这有助于更接近请求的刻度数量。
  };
...
{//每个轴（QCPAxis）都有一个网格实例，使用QCPGrid来绘制网格线，子网线，零线。
  Q_OBJECT
  /// \cond INCLUDE_QPROPERTIES
...
public:
  ...
  */
  ...
  /*!
    ...
    \see setScaleType
  */
  enum ScaleType { stLinear       ///< Linear scaling 线性缩放
  ...
  */
  ...
  //此方法返回 1 或 -1。如果返回 1，则沿轴的正方向移动像素对应于从较低到较高的轴坐标。
  //另一方面，如果该方法返回 -1，则减小像素值对应于从较低到较高的轴坐标。
...
protected:
  ...
  // scale and range:
  QCPRange mRange;
  bool mRangeReversed;//是否需要反向设置坐标
...
void QCPDataContainer<DataType>::add(const QVector<DataType> &data, bool alreadySorted)
{
  ...
  {//如果新数据已排序，且新加键都小于或等于现有
    if (mPreallocSize < n)//如果预分配的大小小于添加的数据的大小
      preallocateGrow(n);
    mPreallocSize -= n;
    std::copy(data.constBegin(), data.constEnd(), begin());
...
void QCPDataContainer<DataType>::preallocateGrow(int minimumPreallocSize)
{
  ...
  int newPreallocSize = minimumPreallocSize;//计算新的池子的大小
  ...
  mData.resize(mData.size()+sizeDifference);//调整容器大小。
...
{
  const int totalAlloc = mData.capacity();//获取容器的总容量
  const int postAllocSize = totalAlloc-mData.size();//未使用的内存
  const int usedSize = size();
  bool shrinkPostAllocation = false;//是否需要释放分配后的内存
  bool shrinkPreAllocation = false;//是否需要释放预分配的内存
...
public:
  ...
  */
  enum RefreshPriority { rpImmediateRefresh ///< Replots immediately and repaints the widget immediately by calling QWidget::repaint() after the replot
                                            ///立即重新绘制并立即重绘小部件 调用QWidget::repaint()
                         ,rpQueuedRefresh   ///< Replots immediately, but queues the widget repaint, by calling QWidget::update() after the replot. This way multiple redundant widget repaints can be avoided.
                                            ///立即重新绘制，通过调用 QWidget::update() 将小部件重绘请求排入队列。这样可以避免多余的小部件重绘。
                         ,rpRefreshHint     ///< Whether to use immediate or queued refresh depends on whether the plotting hint \ref QCP::phImmediateRefresh is set, see \ref setPlottingHints.
                                            ///是否使用立即或排队刷新取决于绘图提示 QCP::phImmediateRefresh 是否设置，请参阅 setPlottingHints。
...
protected:
  // property members:
  QRect mViewport;
  double mBufferDevicePixelRatio;//缓冲像素设备比率
...
public:
  ...
  */
  enum LineStyle { lsNone        ///< data points are not connected with any lines (e.g. data only represented
                                 ...
                                 /// 数据点之间不用线连接，只用符号（由散点样式决定）表示数据。
                   ,lsLine       ///< data points are connected by a straight line
                                 ///数据点之间通过直线连接。
                   ,lsStepLeft   ///< line is drawn as steps where the step height is the value of the left data point
                                 ///线条以阶梯形式绘制，其中阶梯的高度是左侧数据点的值。
                   ,lsStepRight  ///< line is drawn as steps where the step height is the value of the right data point
                                 ///绘制阶梯右侧的步进线，其中阶梯的高度是右侧数据点的值。
                   ,lsStepCenter ///< line is drawn as steps where the step is in between two data points
                                 ///绘制阶梯中心的步进线，其中阶梯的高度是两个数据点值的中间。
                   ,lsImpulse    ///< each data point is represented by a line parallel to the value axis, which reaches from the data point to the zero-value-line
                                 ///每个数据点表示为一条平行于值轴的线段，从数据点延伸到零值线。
                 };
  Q_ENUMS(LineStyle)
