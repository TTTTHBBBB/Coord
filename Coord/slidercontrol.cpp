...
void SliderControl::drawEvent()
{
    ...
    QPoint centerStartPos = CoordSpace::GetCenterPosition(ui->pushButtonLeft);
    int centerStartX = centerStartPos.x();
    int centerStartY = centerStartPos.y();

    QPoint centerEndPos = CoordSpace::GetCenterPosition(ui->pushButtonRight);
...
void SliderControl::collectCentPoint()
{
    ...
    {
        m_labelPointMap[CoordSpace::GetCenterPosition(labelCircle).x()] = labelCircle;
    }
}
...
// 点击左侧按钮
void SliderControl::SlotpushButtonLeft()
{
...
// 点击右侧按钮
void SliderControl::SlotpushButtonRight()
{
